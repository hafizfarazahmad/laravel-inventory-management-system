                    ...
                    @if (isset($purchase))
                        @foreach ($purchase->purchaseItems as $item)
                            <tr>
                                ...
                                <td><input type="number" id="purchase_price" step="0.01" class="form-control"
                                        name="purchase_price[]" value="{{ $item->purchase_price }}"></td>
...
@push('scripts')
    <script>
        ...
        addRowbtn.addEventListener('click', function() {
            const lastRow = purchaseItems.querySelector('tr');
            const newRow = lastRow.cloneNode(true);
            newRow.querySelector('select').selectedIndex = 0;
            newRow.querySelector('input[name="quantity[]"]').value = 1;
            newRow.querySelector('input[name="purchase_price[]"]').value = 0.00;
        ...
        purchaseItems.addEventListener('click', function(e) {
            if (e.target.closest('.removeRow')) {
                ...
                const totalRows = purchaseItems.querySelectorAll('tr').length;
                if (totalRows > 1) {
                    row.remove();
                    calculateGrandTotal();
                } else {
